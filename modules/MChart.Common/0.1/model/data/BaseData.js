;(function(define, _win) { 'use strict'; define( [ 
     ...
     * @class 
     * 一个用于程序的UI组件的基类 
     * 它主要添加一个基于BaseData的实现，让UI组件从Mediators监听 
     * 实现者和监听者是负责匿名事件对象的发送和接受 
     */  
    var BaseData = Objs(
        "Base.model.data.BaseData"
        , {  
            /** 
             * @construct 
             * 初始化一个BaseData实例 
            ...
            /** 
             * 一个BaseData.listenerDescriptor的映射对象 
             * @type {Object} 
             * @private 
             */  
            , listenerMap: null
            ...
            */  
            , dispatchEvent: function( type, properties ) {  
                ...
                if( typeof this.listenerMap[BaseData.QUEUE_PATTERN + type] == 'undefined' )  
                    return;  
              
                var queue = this.listenerMap[BaseData.QUEUE_PATTERN + type].slice(0);  
            ...
            , addEventListener: function( type, listener, context ) {  
                ...
                var newListener = new BaseData.ListenerDescriptor( listener, context );  
              
                var queue;  
                if( typeof this.listenerMap[ BaseData.QUEUE_PATTERN + type ] == "undefined" )  
                    queue = this.listenerMap[ BaseData.QUEUE_PATTERN + type ] = [];  
                else  
                    queue = this.listenerMap[ BaseData.QUEUE_PATTERN + type ];  
            ...
            , removeEventListener: function( type, listener, context ) {
                ...
                var queue = this.listenerMap[ BaseData.QUEUE_PATTERN + type ];  
                var len = queue.length;  
                for( var i = 0; i < len; i++ ) {  
                    var listenerDescriptor = queue[i];  
                    if( listenerDescriptor.equals( new BaseData.ListenerDescriptor( listener, context ) ) ) {  
                        queue.splice(i,1);  
                        return;  
                    }  
                }
     ...
     * @class 
     * @private 
     * Event对象由BaseData类派发到它的事件监听器 
     */  
    BaseData.Event = Objs(
        "demo.view.components.BaseData.Event"
        , {  
            /** 
             * 事件类型 
             * @type {String} 
    ...
    /** 
     * @private 
     * 使用BaseData.listenerMap描述符对象鉴定各个事件监听器 
     * 这是Javascript的内部类 
     */  
    BaseData.ListenerDescriptor = Objs(
        "demo.view.components.BaseData.Event"
        , {  
            /** 
             * @construct 
            ...
            /** 
             * @private 
             * 对比两BaseData.ListenerDescriptor 以确定与目标相同的事件监听器。 
             * 
             * @param {BaseData.ListenerDescriptor} compared 
